...
START_TEST (test_builtin_exit)
{
	...
	scommand_push_back (exit_cmd, bfromcstr ("exit"));
...
START_TEST (test_builtin_chdir)
{
	...
	scommand_push_back (cd_cmd, bfromcstr ("cd"));
	scommand_push_back (cd_cmd, bfromcstr (test_path));
...
START_TEST (test_external_1_simple_parent)
{
	...
	scommand_push_back (ext_cmd, bfromcstr ("command"));
...
START_TEST (test_external_arguments)
{
	...
	scommand ext_cmd = scommand_new ();
	scommand_push_back (ext_cmd, bfromcstr ("command"));
	scommand_push_back (ext_cmd, bfromcstr ("arg1"));
	scommand_push_back (ext_cmd, bfromcstr ("-arg2"));
	pipeline_push_back (test_pipe, ext_cmd);
	/* Queremos que el fork devuelva 0, para testear el hijo */
	mock_fork_setup (pids);
...
static void setup_test_pipe (void) {
	...
	scommand_push_back (ext_cmd, bfromcstr ("command2"));
	scommand_push_back (ext_cmd, bfromcstr ("arg3"));
	pipeline_push_back (test_pipe, ext_cmd);
}
