...
static void check_argument (scommand cmd, const char *first) {
	...
	fail_unless (biseqcstr (arg, first) == 1, NULL);
...
START_TEST (test_command_redir_in)
{
	...
	check_argument (s, "arg1");
	/* con redirección */
	fail_unless (biseqcstr (scommand_get_redir_in (s), "entrada") == 1, NULL);
	fail_unless (scommand_get_redir_out (s) == NULL, NULL);
	/* Y no es en background */
	fail_unless (pipeline_get_wait (output), NULL);
}
...
START_TEST (test_command_redir_out)
{
	...
	/* con redirección */
	fail_unless (scommand_get_redir_in (s) == NULL, NULL);
	fail_unless (biseqcstr (scommand_get_redir_out (s), "salida") == 1, NULL);
	/* Y no es en background */
	fail_unless (pipeline_get_wait (output), NULL);
}
END_TEST
