...
/* Auxiliar, dado que estamos en modo ANSI: */
static char *strdup (const char *s) {
	...
	assert (result != NULL); /* Para los usos de este modulo, no interesa continuar si no hay memoria */
	return result;
}
...
bstring mock_chdir_last = NULL;
int mock_chdir (const char *path) {
	assert (path != NULL);
	mock_counter_chdir++;
	mock_chdir_last = bfromcstr (path);
	errno = ENOENT;
	return -1;
}
